...
FLASH_KERNEL_PARTION_NAME_OLD=ITL-kernel
...
if [ -z $FLASH_RECOVER_ROOTFS_PARTION_NAME ]; then
	FLASH_RECOVER_ROOTFS_PARTION_NAME="rootfs-recover"
else
	FLASH_RECOVER_ROOTFS_PARTION_NAME=$RECOVER_ROOTFS_PARTION_NAME
fi

if [ -z $APP_PARTION_NAME ]; then
	FLASH_APP_PARTION_NAME=app
else
	FLASH_APP_PARTION_NAME=$APP_PARTION_NAME
fi
...
case "$RECOVER_MTD_PART_NAME" in
	"$FLASH_KERNEL_PARTION_NAME_OLD"|"$FLASH_KERNEL_PARTION_NAME"|"$FLASH_ROOTFS_PARTION_NAME"|"$FLASH_APP_PARTION_NAME"|"$FLASH_RECOVER_ROOTFS_PARTION_NAME")
