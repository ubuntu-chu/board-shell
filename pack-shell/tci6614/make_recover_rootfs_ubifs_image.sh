...
UBI_IMAGE_NAME=rootfs-recover.img

UBINIZE_CFG_FILE=ubinize.cfg.rootfs_recover
...
#
RECOVERY_VOL_ID=0
RECOVERY_VOL_IMG_NAME=rootfs-recovery.ubifs.img
#RECOVERY_VOL_IMG_NAME=recovery.ubifs.img
RECOVERY_VOL_SRC_DIR=rootfs_recovery
RECOVERY_VOL_NAME=recover-fs
#20MiB
RECOVERY_VOL_SIZE=20971520
RECOVERY_VOL_FLAGS=autoresize
...
get_leb_cnt $RECOVERY_VOL_SIZE
echo "mkfs.ubifs -r $RECOVERY_VOL_SRC_DIR -F -o $RECOVERY_VOL_IMG_NAME -m $SMALLEST_FLASH_IO_UNIT_SIZE -e $LEB_SIZE -c $leb_cnt"
mkfs.ubifs -r $RECOVERY_VOL_SRC_DIR -F -o $RECOVERY_VOL_IMG_NAME -m $SMALLEST_FLASH_IO_UNIT_SIZE -e $LEB_SIZE -c $leb_cnt
...
echo "vol_flags=$RECOVERY_VOL_FLAGS" >> $UBINIZE_CFG_FILE
